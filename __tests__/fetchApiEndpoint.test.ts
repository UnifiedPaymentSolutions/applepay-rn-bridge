import { getInitPaymentEndpoints, getStartPaymentEndpoints } from '../src/payment/utils';
...
describe('Endpoint utility functions', () => {
  ...
  describe('getInitPaymentEndpoints', () => {
    it('should build the init payment endpoint URL correctly', () => {
      ...
      expect(result.mobileOneoffUrl).toBe(`https://example.com${API_ONEOFF_PATH}`);
    });

    it('should throw if base URL is invalid or empty', () => {
      expect(() => getInitPaymentEndpoints('')).toThrow('Payment link is required');
      expect(() => getInitPaymentEndpoints(null as any)).toThrow('Payment link is required');
    });
  });
  ...
  describe('getStartPaymentEndpoints', () => {
    it('should build all required endpoint URLs correctly', () => {
      ...
      expect(result.paymentMethodsUrl).toBe(`https://example.com${API_PAYMENT_METHODS_PATH}`);
    });
    ...
    it('should throw if base URL is invalid or empty', () => {
      expect(() => getStartPaymentEndpoints('')).toThrow('Payment link is required');
      expect(() => getStartPaymentEndpoints(null as any)).toThrow('Payment link is required');
    });
  });
});